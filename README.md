...
If `SHARPAPI_API_KEY` is set, a sport that fails against The Odds API (quota exhausted, outage, bad key) automatically falls back to [SharpAPI](https://sharpapi.io) for that sport alone (`bot/sharpapi_fetcher.py`) instead of failing the whole fetch -- check `sport_sources` in `logs/odds_fetch_status.json` to see which provider each sport actually came from. SharpAPI's response shape (flat per-selection rows vs. The Odds API's nested bookmakers/markets/outcomes) is reshaped back into the same `market_lines.csv` schema so nothing downstream needs to know which provider a row came from. Confirmed against a real key: mlb/wnba/nfl all return live rows correctly, including the fix for SharpAPI's abbreviated team names (e.g. "BAL Orioles") being reconstructed into the full names ("Baltimore Orioles") the rest of the pipeline expects.
